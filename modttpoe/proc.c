...
#define TTP_STATS64_ZERO(_v) atomic64_set (&ttp_stats._v, 0)
    ...
    TTP_STATS_ZERO (tx_congestion_reduced);
    TTP_STATS_ZERO (rx_congestion_reduced);
    TTP_STATS_ZERO (tx_congestion_echo);
    TTP_STATS_ZERO (rx_congestion_echo);
    TTP_STATS_ZERO (tx_sack_nack);
    TTP_STATS_ZERO (rx_sack_nack);
    TTP_STATS_ZERO (rx_sack_cached);
    TTP_STATS_ZERO (rx_sack_delivered);
    TTP_STATS_ZERO (rx_sack_dropped);
...
#define TTP_STATS64_PRINT(_v) seq_printf (seq, #_v ": %lld\n", atomic64_read (&ttp_stats._v))
    ...
    TTP_STATS_PRINT (tx_congestion_reduced);
    TTP_STATS_PRINT (rx_congestion_reduced);
    TTP_STATS_PRINT (tx_congestion_echo);
    TTP_STATS_PRINT (rx_congestion_echo);
    TTP_STATS_PRINT (tx_sack_nack);
    TTP_STATS_PRINT (rx_sack_nack);
    TTP_STATS_PRINT (rx_sack_cached);
    TTP_STATS_PRINT (rx_sack_delivered);
    TTP_STATS_PRINT (rx_sack_dropped);
